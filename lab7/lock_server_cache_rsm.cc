                                   ...
                                   lock_protocol::xid_t xid, int &)
{
    ...
    if (xid_it == le.highest_xid_from_client.end() || xid_it->second < xid)
    {
        ...
        case LOCKED:
            ...
            revokeQueue.enq(revoke_retry_entry(it->second.owner, lid, it->second.highest_xid_from_client[it->second.owner]));
                                   ...
                                   lock_protocol::xid_t xid, int &r)
{
    ...
    auto xid_it = le.highest_xid_from_client[id];
    if (xid_it != le.highest_xid_from_client.end() && xid_it->second == xid)
    {
        ...
        if (reply_it == le.highest_xid_release_reply.end())
        {
            ...
            case LOCKED_AND_WAIT:
                ...
                retryQueue.enq(revoke_retry_entry(client_need_retry, lid, it->second.highest_xid_from_client[client_need_retry]));
                break;
            case RETRYING:
                ret = lock_protocol::IOERR;
                break;
